...
import { Send, Bot, User, Loader2, Archive, History, ArrowLeft, BookOpen, Settings, Maximize2, Minimize2, Square } from "lucide-react";
...
const ChatTab = () => {
  ...
  const handleSend = async (customContent?: string) => {
    ...
    try {
      ...
      // Check if the error is an abort error (user cancelled)
              ...
              <div className="relative flex items-center gap-3 bg-[#252525] rounded-lg px-4 py-2 max-w-4xl mx-auto border border-white/5">
                ...
                {isLoading ? (
                  <Button
                    ...
                    size="icon"
                    className="w-9 h-9 rounded-lg text-white shrink-0 transition-all duration-300 bg-red-600 hover:bg-red-700"
